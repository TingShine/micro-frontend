...
const subPackageJson = existsSync(subPackageJsonPath) ? readFileSync(subPackageJsonPath, { encoding: 'utf8', flag: 'r' }) : {}
...
const externalsStr = Object.keys(externalsObj).map(dependency => {
  const version = dependency in subDependenciesObj ? subDependenciesObj[dependency] : mainDependenciesObj[dependency]
  return `npm/${dependency}@${version}`
}).join(',')
...
export const externalsScript = `<script src='${cdnPath}'></script>`

// combine方案
const downloadUrl = `https://cdn.jsdelivr.net/combine/${externalsStr}`
console.log(`Downloading Resource ==> ${downloadUrl}`);
const resources = await axios.get(downloadUrl).then(response => response.data)
